...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void) {
    ...
    switch(track) {
        case TRACK_INTRO:
//            sample = ((t >> 10) & 42) * t; // put bytebeat here

            // uses mod and thus is very slow
            // sample = (t%25-(t>>2|t*15|t%227)-t>>3)|((t>>5)&(t<<5)*1663|(t>>3)%1544)/(t%17|t%2048);

            // works well            
            sample = (t>>7|t|t>>6)*10+4*(t&t>>13|t>>6);
            
            break;
        case TRACK_EXAMPLE1:
            sample = sample = ((t >> 10) & 42) * t; // put bytebeat here
            break;
        case TRACK_EXAMPLE2:
